...
### Runtime remediation gate

The first live Breath gate exposed a runtime-only gap: an occupied shelter could lead the brain into `INVESTIGATE`, while The Stop admitted only patrol/search/return states. The policy now permits every non-chase state, cancels the current move, zeros rig velocity, and temporarily freezes/orients the unanchored Humanoid. Controlled policy regressions cover investigation, chase, Draw commitment, cooldown, range, and occupancy; repeatability still requires five fresh unanchored Studio observations.

Each shelter now has an authored `HidingCamera` and outward `ExitPoint` generated with the source-owned world, while `HunterApproach` remains solely the Listener inspection target. This separates concealed presentation, survivor release, and hunter navigation. Existing shelter capacities remain one to three. Break-and-run survival through three exits remains a runtime gate, not a source-level claim.

Freeform stationary pressure is now legible before it is consequential: a storm-exposure border and warning start at six seconds, evidence starts at twelve, and movement clears the presentation. Touch controls use smaller separated slots and lift the Breath panel above them. The rejected packaged `swoosh.wav` reference was replaced with an approved Roblox built-in sound, recap output suppresses missing/zero break durations, and ContextAction bindings are tracked before cleanup. None of these changes substitutes for phone screenshots, fresh runtime logs, or multiplayer staging evidence.

Fresh local Studio evidence on the remediation build records five successful Stops across five distinct occupied shelters. Every attempt used the live Humanoid unanchored; `Listening` became true, `AutoRotate` became false, and the rig retained the same horizontal world position during the sampled hold. Separate clean-round route checks exited through the production `VOLUNTEER OUT` action at Warning, Critical, and an active Stop. Each server-accepted exit moved the avatar `15.13` studs through the authored lobby route and sampled `100` health immediately after the move. These controlled outcomes prove that leaving is not an automatic capture; longer pursuit balance remains a blind-play question.

The six-second **THE WIND IS FINDING YOU** cue and twelve-second **MOVE — THIS SPOT IS EXPOSED** consequence were captured from the real client before and after server evidence began. Samsung Galaxy A06 and tablet simulator captures show the Breath panel above the action region; PEEK and VOLUNTEER OUT use separate compact slots. The exact final Rojo build then launched in a clean Studio process with an empty error/warning log (analytics information messages only). No multiplayer claim is derived from this solo runtime evidence.

## Visual readability and cinematic lighting remediation

The source-authored resort now separates its lighting into a warm main-practical layer and a restrained amber/green emergency layer. Normal storm lighting raises shadow detail, reduces atmosphere density and haze, removes the heavy blue/desaturated grade, and keeps exterior ambience cooler than the interior. Main lobby, shelter wing, service corridor, generator/fuse, pool route, and extraction approach each have motivated focal lighting rather than uniform global fill.

The blackout state now disables main practicals while retaining low-output emergency/navigation lights. Emergency fixtures use an explicit `LightingRole` attribute instead of relying only on names. Bloom is restrained, gameplay depth of field is explicitly disabled, and reflective response is limited to wet ground, pool water, and puddles. The Listener remains unlit, but its lifted body values, corroded-metal fins, and thin shoulder contours catch nearby practicals and lightning for silhouette separation.

These source and state changes are covered by formatting, lint, unit, structure, and Rojo build validation. They still require a fresh Studio visual pass and a new OBS comparison before visual acceptance.
